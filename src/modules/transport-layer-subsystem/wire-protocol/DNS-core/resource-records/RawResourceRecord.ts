import type { DNS_CLASSES } from '../constants/DNS_CLASSES.js';
import type { ResourceRecord } from './ResourceRecord.js';

export class RawResourceRecord implements ResourceRecord<Buffer> {
  readonly name: string;
  readonly type: number;
  readonly RR_class: DNS_CLASSES;
  readonly ttl: number;
  readonly rdLength: number;
  readonly rData: Buffer;

  constructor(name: string, type: number, RR_class: DNS_CLASSES, ttl: number, rdLength: number, rData: Buffer) {
    this.name = name;
    this.type = type;
    this.RR_class = RR_class;
    this.ttl = ttl;
    this.rdLength = rdLength;
    this.rData = rData;
  }
}
