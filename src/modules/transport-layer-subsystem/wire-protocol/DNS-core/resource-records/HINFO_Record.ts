...
export class HINFO_Record implements ResourceRecord<RDataMap[DNS_TYPES.HINFO]> {
  readonly name: string;
  readonly type: DNS_TYPES.HINFO;
  readonly RR_class: DNS_CLASSES;
  readonly ttl: number;
  readonly rdLength: number;
  readonly rData: RDataMap[DNS_TYPES.HINFO];

  constructor(
    name: string,
    type: DNS_TYPES.HINFO,
    RR_class: DNS_CLASSES,
    ttl: number,
    rdLength: number,
    rData: RDataMap[DNS_TYPES.HINFO]
  ) {
    this.name = name;
    this.type = type;
