...
`alpha` releases are not always required, so a release chain may not include `alpha` releases. It will however, **_always_** include at least one `beta` and one `rc` release before a `stable` release is published.
